    ...
    <div class="container">
        <div class="timeline">
            <div
                ...
                :class="{ warning: hit.oro }"
                v-for="hit in hits"
                :key="hit.id"
            >
                <div class="timeline-icon">
                    <img
                        :src="hit.user.avatarUrl"
                        :alt="hit.user.displayName"
                        class="avatar"
                    />
                    <span v-if="hit.oro" class="oro">🥇</span>

                    <!-- <i class="far fa-grin-wink"> </i> -->
                </div>
                <div class="timeline-body">
                    <h4 class="timeline-title" v-if="hit.oro">
                        <span class="badge">Oro!</span>
                    </h4>
                    <!-- <p>
                        {{ hit.user.displayName }}
                    </p> -->
                    <p class="timeline-subtitle">
                        {{ moment(hit.en).fromNow() }}
                    </p>
                </div>
            </div>
        </div>
